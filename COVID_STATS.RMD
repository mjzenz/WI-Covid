...
PLOT  + geom_line(aes(y = AVG_7_CHANGE_DEATHS), color = "black") + 
  ...
  scale_x_date(limits = Date_lims) +
  labs(y = "", title  = "Daily Covid-19 Deaths", subtitle =  "7-day AVG in black, today's value in blue") 
...
* **NEW DEATHS** are doubling every `r round(Death_Doubling, 0)` days.  

* **NEW HOSPITALIZATIONS** are doubling every `r round(Hosp_Doubling, 0)` days.

```{r NEW_DEATHS_STATE_LOG, echo=FALSE, warning=FALSE}

PLOT <- ggplot(STATEWIDE_VALUES_VALID, aes(x = Date))
PLOT  + geom_point(aes(y = CHANGE_DEATHS), color = "red", fill = "red") +  
  stat_smooth(aes(y = CHANGE_DEATHS), method = "lm",formula = y~x,  col = "red") +
 geom_point(aes(y = CHANGE_HOSP), color = "blue", fill = "blue") +  
  stat_smooth(aes(y = CHANGE_HOSP), method = "lm",formula = y~x, col = "blue") +
  scale_y_log10()+
  scale_x_date(limits = date(c(lm_DATE_MIN, MAX_DATE))) +
  labs(y = "New Hosp./Deaths", title  = "Exponential Growth in Daily New Hospitalizations and Deaths", subtitle =  "Hosp. in blue, Deaths in red, line slope indicates exp. growth rate") 
...
PLOT  + geom_line(aes(y = AVG_7_CHANGE_POS), color = "black") + 
  ...
  #scale_x_date(limits = Date_lims) +
  labs(y = "", title  = "Daily WI Covid-19 Cases", subtitle =  "7-day AVG in black, today's value in blue") 
...
PLOT + geom_bar(position = "stack", stat = "identity") + 
  scale_x_date(limits = Date_lims) +
  labs(y = "", title  = "New Daily Covid-19 Cases") 
```
...
PLOT  + geom_line(aes(y = AVG_7_CHANGE_POS), color = "black") + geom_point(aes(y = CHANGE_POS), fill = "red" ,color = "red") + geom_point(aes(y = MAX_POS), fill = "blue" ,color = "blue") + 
  scale_x_date(limits = Date_lims) +
  facet_wrap(~ COUNTY, ncol = 2, scales = "free")   +  labs(y = "", title  = "Daily CTY Cases (7-day AVG in black, today's value in blue)") 
