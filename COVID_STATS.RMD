...
PLOT  + geom_line(aes(y = AVG_7_CHANGE_DEATHS), color = "black") + 
  ...
  labs(y = "", title  = "Daily Covid-19 Deaths, Last 3 Months", subtitle =  "7-day AVG in black, today's value in blue") 

```

```{r NEW_DEATHS_STATE_LOG, echo=FALSE, warning=FALSE}

PLOT <- ggplot(STATEWIDE_VALUES, aes(x = Date))
PLOT  + geom_point(aes(y = log_CHANGE_DEATHS), color = "red", fill = "red") +  
  stat_smooth(aes(y = log_CHANGE_DEATHS), method = "lm", col = "red") +
 geom_point(aes(y = log_CHANGE_HOSP), color = "blue", fill = "blue") +  
  stat_smooth(aes(y = log_CHANGE_HOSP), method = "lm", col = "blue") +
  scale_x_date(limits = date(c(lm_DATE_MIN, "2020-10-29"))) +
  labs(y = "", title  = "Daily Covid-19 Deaths, Last 3 Months", subtitle =  "7-day AVG in black, today's value in blue") 
...
PLOT  + geom_line(aes(y = AVG_7_CHANGE_HOSP), color = "black") + 
  ...
  scale_x_date(limits = Date_lims) +
  labs(y = "", title  = "New Covid-19 Hospitalizations, Last 3 Months", subtitle = "7-day AVG in black, today's value in blue") 
